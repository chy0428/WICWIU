...
*/
template<typename DTYPE> class LossFunction : public Operator<DTYPE>{
private:
    ...
    // Operator<DTYPE> *m_pInputOperator;  =>  *(this->GetInput())
...
// 오상진 추가
template<typename DTYPE> Operator<DTYPE> *LossFunction<DTYPE>::SetInput(Operator<DTYPE> *pInput) {

    this->GetInputContainer()->Push(pInput);
    // m_InputDegree++;
...
template<typename DTYPE> int LossFunction<DTYPE>::Alloc(Operator<DTYPE> *pOperator, Operator<DTYPE> *pLabel) {
    ...
    //this->SetResult(pOperator);
    // (this->GetInput())->Push(pOperator);   // m_apInput
    // m_pInputTensor   = *(this->GetInput())->GetResult();
...
//     return (*m_aaResult)[0];  //  과거 : m_aResult;
// }
...
template<typename DTYPE> Operator<DTYPE> *LossFunction<DTYPE>::GetOperator() {
    return *(this->GetInputContainer()->GetRawData());
}

template<typename DTYPE> Tensor<DTYPE> *LossFunction<DTYPE>::GetTensor()  {
    return (*( this->GetInputContainer()->GetRawData() ))->GetResult();
}
...
@return NULL
*/
template<typename DTYPE> int LossFunction<DTYPE>::ForwardPropagate(int pTime) {
    #ifdef __DEBUG__
...
@return NULL
*/
template<typename DTYPE> int LossFunction<DTYPE>::BackPropagate(int pTime) {
    #ifdef __DEBUG__
...
@return NULL
*/
template<typename DTYPE> int LossFunction<DTYPE>::ForwardPropagateOnGPU(int pTime) {
    # if __DEBUG__
    std::cout << this->GetName() << '\n';
    # endif // __DEBUG__
    return 0;
...
@return NULL
*/
template<typename DTYPE> int LossFunction<DTYPE>::BackPropagateOnGPU(int pTime) {
    # if __DEBUG__
    std::cout << this->GetName() << '\n';
    # endif // __DEBUG__
    return 0;
...
template<typename DTYPE> DTYPE& LossFunction<DTYPE>::operator[](unsigned int index) {
    return (*this->GetResult())[index];       // (*m_aResult)[index];
}
